...
import com.wondersgroup.roa.AnnotationServletServiceRouterFactoryBean;
...
public class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		if (StringUtils.hasText(threadFerryClassName)) {
			try {
				...
				if (!ClassUtils.isAssignable(ThreadFerry.class, threadFerryClass)) {
					throw new ROAException(threadFerryClassName + "没有实现" + ThreadFerry.class.getName() + "接口");
				}
		...
		if (StringUtils.hasText(serviceTimeoutSeconds)) {
			serviceRouterDef.getPropertyValues().addPropertyValue("serviceTimeoutSeconds",
					Integer.parseInt(serviceTimeoutSeconds));
		}

		String uploadFileMaxSize = element.getAttribute("upload-file-max-size");
		if (StringUtils.hasText(uploadFileMaxSize)) {
			serviceRouterDef.getPropertyValues().addPropertyValue("uploadFileMaxSize",
	...
	private RuntimeBeanReference getInvokeTimesController(Element element, Object source, ParserContext parserContext) {
		...
		if (element.hasAttribute("invoke-times-controller")) {
			...
			if (logger.isInfoEnabled()) {
				logger.info("ROA装配一个自定义的服务调用次数/频度控制器:" + invokeTimesControllerRef.getBeanName());
			}
		}
		else {
			...
			String invokeTimesControllerName = parserContext.getReaderContext().registerWithGeneratedName(
					invokeTimesControllerDef);
			parserContext.registerComponent(new BeanComponentDefinition(invokeTimesControllerDef,
					invokeTimesControllerName));
	...
	private RuntimeBeanReference getServiceAccessController(Element element, Object source, ParserContext parserContext) {
		...
		if (element.hasAttribute("service-access-controller")) {
			...
			if (logger.isInfoEnabled()) {
				logger.info("ROA装配一个自定义的服务访问控制器:" + serviceAccessControllerRef.getBeanName());
			}
		}
		else {
			...
			String serviceAccessControllerName = parserContext.getReaderContext().registerWithGeneratedName(
					serviceAccessControllerDef);
			parserContext.registerComponent(new BeanComponentDefinition(serviceAccessControllerDef,
					serviceAccessControllerName));
	...
	private RuntimeBeanReference getAppSecretManager(Element element, Object source, ParserContext parserContext) {
		...
		if (element.hasAttribute("app-secret-manager")) {
			...
			if (logger.isInfoEnabled()) {
				logger.info("ROA装配一个自定义的密钥管理器:" + appSecretManagerRef.getBeanName());
			}
		}
		else {
	...
	private RuntimeBeanReference getSessionManager(Element element, Object source, ParserContext parserContext) {
		...
		if (element.hasAttribute("session-manager")) {
			...
			if (logger.isInfoEnabled()) {
				logger.info("ROA装配一个自定义的SessionManager:" + sessionManagerRef.getBeanName());
			}
		}
		else {
	...
	private RuntimeBeanReference getConversionService(Element element, Object source, ParserContext parserContext) {
		...
		if (element.hasAttribute("formatting-conversion-service")) {
			...
			if (logger.isInfoEnabled()) {
				logger.info("ROA装配一个自定义的FormattingConversionService:" + conversionServiceRbf.getBeanName());
			}
		}
		else {
	...
	private RuntimeBeanReference getROAValidator(Element element, Object source, ParserContext parserContext) {
		RuntimeBeanReference roaValidatorRbf;
		if (element.hasAttribute("roa-validator")) {
			roaValidatorRbf = new RuntimeBeanReference(element.getAttribute("roa-validator"));
			if (logger.isInfoEnabled()) {
				logger.info("ROA装配一个自定义的ROAValidator:" + roaValidatorRbf.getBeanName());
			}
		}
		else {
			...
			String conversionName = parserContext.getReaderContext().registerWithGeneratedName(conversionDef);
			parserContext.registerComponent(new BeanComponentDefinition(conversionDef, conversionName));
			roaValidatorRbf = new RuntimeBeanReference(conversionName);
			if (logger.isInfoEnabled()) {
				logger.info("使用默认的ROAValidator:" + DefaultSecurityManager.class.getName());
			}
		}
		return roaValidatorRbf;
	}
